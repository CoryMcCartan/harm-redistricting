...
plot_state = function(map, people, plan=NULL, qty=NULL, ppl_size=1.65, grey_out = FALSE,
    ...
    p = redist.plot.map(map, fill=qty, ...) +
        ...
        geom_sf(data=map, size=0.55, fill=NA, color="white") +
    ...
    if (!is.null(plan)) {
        distrs = map %>%
            ...
            group_by(.plan) %>%
            summarize()
        p = p + geom_sf(data=distrs, fill=NA, color="black", size=1.0)
    }
    p
}
...
for (i in 1:3) {
    p2 = add_lab(p2, fmt_lean(dem_gerry_dem[as.character(i)]),
...
p3 = plot_state(minissouri, minissouri_people,
                pl_fair, dem_fair)
for (i in 1:3) {
    p3 = add_lab(p3, fmt_lean(dem_fair[as.character(i)]),
                 coords_fair$x[i], coords_fair$y[i])
}

p4 <- plot_state(map = minissouri,
...
p5 = ggplot(d_harm, aes(x=type, y=harm, fill=group)) +
...
p1 + p2 + p3 + p4 + p5 + plot_layout(nrow=1, widths=c(0.21, 0.21, 0.21, 0.21, 0.16)) &
    guides(fill="none")

ggsave(here("pres/minissouri.pdf"), width=11, height=2.5)
