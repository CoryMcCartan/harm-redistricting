...
#'   second row is out-group.
harm = function(plans, group_share, dvote, rvote,
                idx_1=seq_len(ncol(as.matrix(plans))),
                idx_2=seq_len(ncol(as.matrix(plans))),
                ker=k_step) {
    x = matrix(
        ker(eval_tidy(enquo(group_share), plans)),
        nrow=attr(plans, "ndists")
    )
    harm_helper(as.matrix(plans), x, dvote, rvote, idx_1, idx_2)
}
...
# plans calculator
calc_plans_stats = function(plans, map, dem, gop, ker=k_t(), max_harm=200L) {
    ...
    hh = harm(plans, dem, dvote, rvote, idx_2=idx_2, ker=ker)
    pl_sum$h_dem = hh[1, ]
    pl_sum$h_rep = hh[2, ]
    pl_sum$dh = hh[1, ] - hh[2, ]
    pl_sum$h = hh[3, ]
