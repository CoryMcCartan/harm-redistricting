...
authors:
  - name: Cory McCartan
    ...
    department: Ph.D. Candidate, Department of Statistics
    affiliation: Harvard University
    location: 1 Oxford St, Cambridge, MA
    email: cmccartan@fas.harvard.edu
  - name: Christopher T. Kenny
    department: Ph.D. Candidate, Department of Government
    affiliation: Harvard University
    location: 1737 Cambridge St., Cambridge, MA
...
abstract: |
  ...
  through analyses of partisan gerrymandering in New Jersey and voting rights 
...
Sections \ref{sec:alabama} demonstrates this approach in practice.
...
In *Allen,* the plaintiff's expert, Moon Duchin, submitted four such remedial plans, one of which is shown in Figure \ref{fig:al-maps}(e).
...
In Appendix \ref{app:alsampling}, we repeat the analysis performed here on a different counterfactual consisting of plans sampled with a redistricting simulation algorithm; the results are broadly similar.
...
As Section \ref{sec:nj} and \ref{sec:alabama} showed, this opens up space for more nuanced analyses that can extend to the local level as well.
...
Here, we perform the same analysis as in Section \ref{sec:alabama}, but using a randomly sampled set of plans as a counterfactual, rather than the four plans used in litigation.
...
```{r alabama-harm-sample, fig.width=6.5}
#| fig.cap: |
...
Figure \ref{fig:alabama-harm-sample} summarizes the results, which are mostly similar to the results in in Section \ref{sec:alabama}.
