...
This paper addresses all of these limitations with a new framework of individual and differential harm for the unified study of all forms of gerrymandering.
...
While many existing partisan gerrymandering metrics make use of an *implicit* counterfactual plan, it may not be of substantive interest or even an actually feasible plan at all.
...
In Section \ref{sec:alabama}, we study voting rights litigation for Alabama congressional districts, and use differential harm to separate the racial and partisan effects of the enacted districting plan.
...
Every voter resides in a legislative district., Each assignment defines a *districting plan* and is represented here by a function $q(i)$ indicating the district that individual $i$ lives in.
The set of all possible districting plans is $\Plans$.

For an election $v$, we tally the votes for each candidate; the candidate with the most votes wins.
...
First, it considers only individuals who vote: people who abstain from voting have experience zero harm.
...
Naturally, extending this approach requires auxiliary data to implement, such as a large-scale survey, as candidate preferences for non-voters cannot be directly inferred from election returns.
...
Despite being only a scalar quantity, $H(q)$ actually fully captures the harm created by a plan $q$.
...
As expected one dimension (the first) primarily aligns with a traditional liberal-conservative spectrum, with progressives like Bernie Sanders, Elizabeth Warren, and current Boston mayor Michelle Wu on the left, and centrists like Joseph R. Biden and Wu opponent Annissa Essaibi-George on the right.
...
These estimates were derived through multilevel regression and post-stratification from survey data, calibrated to actual election results.
