...
The efficiency gap and differential partisan harm are often highly correlated (though the efficiency gap is much more discrete), as wasted votes and partisan harm are rooted in similar logic.[^7]
...
Since 1995, New Jersey congressional redistricting has been handled by a 13-member commisison, with 6 political appointees from each party and one independent chair with *de facto* tiebreaking authority.
For the 2020 redistricting cycle, each party submitted a proposed plan to the chair, John Wallace Jr. Wallace ultimately selected the Democratic plan for enactment, "simply because in the last redistricting, the map was drawn by Republicans."[^8] Here, in contrast, we evaluate the two plans on the basis of existing gerrymandering metrics as well as through the proposed harm framework.
...
For each of these sampled plans and the two gerrymanders, we calculate the average harm and differential partisan harm versus the simulated baseline,[^10] as well as the expected number of Democratic seats, the efficiency gap, partisan bias, the mean-median score, and partisan declination.
...
The estimate of differential partisan harm aligns with the expected partisan skew of each plan, with a value of $-0.036$ for the Democratic proposal and $0.020$ for the Republican proposal.
These values are relatively directly interpretable; for example, the differential harm score of $-0.036$ means that an average Democratic voter is 3.6% less likely to be misrepresented than their average Republican counter-partisan.
...
The sampled plan which minimizes differential harm yields 8.18 expected Democratic seats, compared to a sample average of 8.41 seats, and the 6.8 seats that would be required for an outcome completely proportional to the parties' statewide vote share.

Both proposals harm the average voter at a higher rate than most of the sampled plans, with the Republican plan again more extreme in this regard ($p=0.0008$ versus $p=0.061$ for the Democratic plan).
...
Minimizing bias with other measures, however, could lead one astray: plans which score an efficiency gap of zero can have average harm as high as 0.172 and differential harm as extreme as $-0.061$, nearly twice the value of the Democratic proposal.
Plans with zero partisan bias or mean-median score likewise have higher-than-average harm and tend to differentially harm Republicans.
...
Across the state, the estimated average harm is 0.079, which translates to around 159,000 voters.
...
Normalizing by group sizes, the expected harm for a Black Democrat is 0.22, for a Black Republican is 0.037, for a White Democrat is 0.083, and for a White Republican is 0.017.
Thus while the challenged plan differentially harms both Black voters and Democratic voters, it *especially* harms Black Democrats, who have a differential harm of 0.138 versus White Democrats and a differential harm of 0.183 versus Black Republicans.
...
We substitute these point estimates directly into our election model, which fails to account for uncertainty in the estimates.
