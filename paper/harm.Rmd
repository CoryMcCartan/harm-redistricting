...
Partisan gerrymandering, whether in the form of rotten boroughs or Elbridge Gerry's salamander-shaped district [@griffith1907rise, pp. 72--73], is perhaps the most consistently visible manifestation of this tendency.
The academic study of partisan gerrymandering has largely centered on how electoral systems treat political parties, as captured in the *seats-votes curve* or notions of *partisan symmetry* [@tufte1973pb; @grofman1983measures; @king1987democratic; @king1989representation; @gelman1994unified; @thomas2013estimating; @katz2020].
Other types of gerrymandering exist as well.
In the United States, many states have historically used racial gerrymandering to maintain White political supremacy [@gilliland1969racial].
The Voting Rights Act of 1965 (VRA) provided a framework for legal challenges to racial gerrymanders that has also shaped the academic literature, which is characterized by a focus on identifying the voting preferences of racial groups and observing the extent to which these are realized or frustrated through district boundaries and other features of the electoral system [@oloughlin1982id; @grofman1991identifying; @king1997solution; @greiner2006ecological].
At the local level, where most redistricting occurs, partisan dynamics are not always as salient, and the manipulation of the electoral system may be more subtle or may favor a narrower or less well-defined group [@cain2002mapmaking; @halberstam2012process].
Local redistricting has been less extensively studied, in part for this reason and in part because of data limitations [but see @siegel2013educational; @vargas2021racial].
...
To motivate individual and differential harm, we use *Minissouri*, a stylized representation of Missouri.
The state's total population is 180, and it should be divided into three districts with population between 54 and 66.
Minissouri has two medium-sized cities and a substantial rural population which an overall partisan breakdown of 55.6% R -- 44.4% D.
...
Now suppose the redistricting plan in Figure \ref{fig:minissouri}(a) is the enacted plan.
Despite the statewide popular vote favoring Republicans, under this plan Democrats win two of the three seats.
In contrast, a more proportional plan \ref{fig:minissouri}(b) elect two Republicans and one Democrat.

Our intuition tells us that plan (a) unfairly packs Republicans into a rural district, depriving them of one seat.
...
Here, then the partisan differential harm of plan (a) is $-41.5\%$, meaning that the average Republican voter in Mininois is 41.5 percentage points more likely to be represented by a Democrat than the average Democrat is to be represented by a Republican, compared to plan (b).

We need not limit our consideration to partisanship.
We can also tally harm by *race*, which in Minissouri is correlated with both geography and partisanship, finding that 13 circle voters and 33 square voters are harmed.
This corresponds to a differential racial harm of $-20\%$, where positive values indicate pro-square bias.
Square voters are 20 percentage points more likely to be harmed than circle voters.
While this number cannot establish whether the drawers of plan (a) intended to harm square voters, or merely aimed to maximize Democratic representation, it does quantify the impact of plan (a) on these voters versus the alternative plan (b).
...
Individuals vote for a candidate to represent districts.
Districts are indexed by $\Distrs$.
...
Every voter resides in a legislative district.,
Each assignment defines a *districting plan* and is represented here by a function $q(i)$ indicating the district that individual $i$ lives in.
The set of all possible districting plans is $\Plans$.

For election $v$, we tally the votes for each candidate.
...
The key intuition from Section \ref{sec:motivation} is that *to be harmed*, a voter must have a better outcome in the counterfactual world where a different decision was made.
To operationalize this, we rely on a counterfactual districting plan $\tilde q$.
Then we define the *harm* to an individual $i$ in election $v$ held under districting plan $q$ as 

$$h_i(v,q,\tilde q) = \ind\{v(i) \neq \win_i(v, q)\} \ind\{v(i) = \win_i(v, \tilde q)\}.$$

A voter is harmed only if they are not represented by the candidate of their choice under $q$ but they would have been represented by their candidate of choice under the counterfactual plan $\tilde q$.
...
In practice, voters, courts, or politicians may find different balances in preventing harm and maximizing gains.

Our definition of harm is limited in two notable regards.
First, it considers only individuals who vote: people who abstain from voting have a harm of zero.
People who do not voter express no formal preference in electoral outcomes, even if they have informal preferences.
The natural theoretical extension of this definition is to then separate individual preferences from vote choice. 
The latter defines election winners, but the former provides a more-broad harm measure.
Naturally, extending this approach requires auxiliary data to implement, such as an absurdly large-scale survey, as candidate preferences for non-voters are not directly inferred from election returns.
As surveys are typically benchmarked against electoral outcomes, we do not endorse this type of extension.

A second limitation is that $h_i(v,q,\tilde q)$ ignores any preferences a voter may have between the candidates that he or she did not vote for.
...
This reflects the simplistic nature of the cost function assumed in single-choice elections.
The problem could be remedied by replacing the indicator functions in the harm measure with other cost functions that consider ordinal preferences.
...
Ideally, we would average individual harm (1) over a set of future elections under the enacted districting plan and (2) over a set of hypothetical districting plans which are consistent with districting criteria.
We could even consider a set of individuals with some shared characteristics.
...
The random variable $V$ can be conceptualized as ranging over possible future elections---one realization could correspond to a heavily Democratic-favoring midterm year, while another could involve incumbents underperforming particularly badly.
The random variable $\tilde Q$ captures the range of potential counterfactual districting plans.
Rarely would an analyst have a single counterfactual plan in mind.
Different realizations of $\tilde Q$ correspond to different districting plans that are permissible under the relevant districting criteria.
...
If we treat $V$ and $\tilde Q$ as random variables, we can then condition on particular values of each.
...
$H(q)$ describes the harm created by a plan $q$.
...
Indeed, preserving "communities of interest" is one of the most common statutory and constitutional requirements imposed by states on the plan-drawing process [@ncslcriteria].
...
Fairness is a strict condition: real-life plans are incredibly unlikely to be exactly fair.
...
[^2]: For this, we use the baseline count of Democratic or Republican votes.
    This is a slight simplification, since the vote share changes across random draws, the number of Democratic and Republican voters changes too.
    To deal with this issue completely necessitates modeling elections at the precinct level, which requires more data and computational costs.

# Connections to Current Practice for Analyzing Partisan and Racial Gerrymandering {#sec:connections}

A primary advantage of our proposed harm measures is that they our measures are defined at the individual level.
This allows for the study of redistricting far beyond racial and partisan gerrymandering.
The measures provide an improvement over existing analytical approaches in both of these important contexts.
...
The modern analysis of vote dilution and racial gerrymandering are inseparable from the Voting Rights Act of 1965 (VRA) and the 14th and 15 Amendments.
...
Plugging in past election results without using an election model places assumes too much about the stability of electoral dynamics and consequently can significantly understate the uncertainty in how a particular districting plan will perform into the future.
...
Indeed, @katz2020 reference an implicit counterfactual in discussing the normative implications of partisan symmetry, writing that "[o]bviously, a process designed to be 'fair' that turns out to be substantially asymmetric [i.e., have nonzero partisan bias], if there is a fairer alternative, would not normally be regarded as a legitimate outcome." 
...
The efficiency gap and is often highly correlated with estimates of differential partisan harm under a stochastic uniform partisan swing model (though the efficiency gap is much more discrete), as wasted votes and partisan harm are rooted in similar logic.[^6]
...
In the worst cases, this can lead to directionally wrong conclusions, as we will see in the next section.
...
\end{align*} Here, $F_j$ is a matrix of preferences by race and latent dimension within precinct $j$, so $F_{j}^\top(P_j\turn_j)^\top$ is a vector with elements containing the preferences of the precinct's voters in each latent dimension, after marginalizing out race.
...
As expected one dimension (the first) primarily aligns with a traditional liberal-conservative spectrum, with progressives like Bernie Sanders, Elizabeth Warren, and current Boston mayor Michelle Wu on the left, and centrists like Joseph R. Biden and Wu opponent Anissa Essaibi-George on the right.
The second dimension appears primarily to capture racial differences in support---as the factor value maps in Figure \ref{fig:boston-factors} show, minority-majority and White areas of the city take opposite values on the second dimension.
...
The accuracy of these validation predictions tracks with previous work that also used factor models to predict runoff outcomes after a first-stage election [@kamakura2006modeling].
...
Comparing the enacted plan to the the simulations shows who is harmed by the current plan versus a race-neutral baseline.
The reverse comparison shows who would be harmed by a race-neutral baseline in comparison to the current plan.
...
And while modern simulation methods are a valuable starting point to defining counterfactuals, these methods involve analyst discretion and interpretation of often-murky legal standards.
...
Because each precinct has dozens or hundreds of voters, a Normal approximation to the Binomial likelihood is appopriate, and so we model the vote shares $s_{jk}$ as \begin{align*}
