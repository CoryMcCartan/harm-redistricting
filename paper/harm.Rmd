...
For example, in Massachusetts, it was impossible to draw a set of congressional districts that produces even one reliable Republican seat, despite Republican voters making up roughly 32% of the state's voters [@duch:18].
...
Once a set
of districting rules has been agreed on, what matters is not how a plan measures up to a fixed ideal, but how it performs in comparison to counterfactual plans.
...
Minissouri has two medium-sized cities and a substantial rural population which an overall partisan breakdown of 55.6% R to 44.4% D.
...
In direct terms, this means, on account of their partisan affiliation, Democrats would be 41.5% less likely to be represented by a Democrat than Republicans are to be represented by a Republican.
...
A voter is harmed only if they are not represented by the candidate of their choice under $q$ (i.e. $\ind\{v(i) \neq \win_i(v, q)\}$) but they would have been represented by their candidate of choice under the counterfactual plan $\tilde q$ (i.e. $\ind\{v(i) = \win_i(v, \tilde q)\}$).
...
Ideally, we would average individual harm over a set of future elections under the enacted districting plan and over a set of hypothetical districting plans which are consistent with districting criteria.
...
In the algorithmic fairness literature, the implicit counterfactual is a perfect predictor and the actual decision process harms individuals whenever it makes a false positive or false negative error.
...
For example, one could operate on the logit scale, model incumbency effects, or add terms to account for local factors.
...
As such, we can study redistricting for any defined group memberships.
For social science, perhaps the two most important group memberships are that of race and party.
For these important contexts, we begin by showing that our measures provide an improvement over existing analytical approaches.
...
We can account for geographic sorting, political cohesion, and the individual preferences of every voter.
Sections \ref{sec:alabama} and \ref{sec:boston} demonstrate this approach in practice.

## Partisan Gerrymandering

The canonical framework for considering partisan fairness in redistricting is partisan symmetry [@king1987democratic]. 
Partisan symmetry implies that the seats $s$ won by a party with vote share $v$ would be satisfied if $s(v) = 1 - s(1 - v)$. 
In practice, the partisan symmetry framework is applied via the partisan bias metric $\beta(v)$, which measures the seat deviation from symmetry [@tufte1973pb; @katz2020], i.e., the number of seats that would need to be reallocated to reach partisan symmetry for a given statewide vote share $v$ (usually $\half$).
...
[^3]: Wasted votes are those cast for a losing candidate, as well as those cast for a winning candidate beyond the required 50% to win the election. They are exactly the votes which don't contribute to winning a seat.
...
To apply our proposed framework to study the council district boundaries, we predict individual voting preferences under counterfactual districting plans.
This is more difficult than in both New Jersey and Alabama, for two reasons.
First, the city council is officially nonpartisan, but city politics are dominated by the Democratic Party, so voting preferences are more complex than a simple partisan preference.
...
Yet, the model is a useful simplification of reality which captures the major electoral dynamics relevant to our question of interest.
...
The latent factor ecological inference model is not necessary for this kind of analysis; local survey data with ideological preferences and racial information would also suffice.
...
Our hypothetical elections are focused on cases with two candidates, to simplify the challenges of measuring harm with multiple candidates, over which voters may have different ranked preferences.
...
Recall that harm is asymmetric.
While that may be an undesirable property in some areas, here we can leverage that fact.
We flip the setup and compute harm by averaging over the simulated plans with the enacted plan as the counterfactual.
Here, comparing the enacted plan to the the simulations shows who is harmed by the current plan versus a race-neutral baseline.
...
And while modern simulation methods are a valuable method for defining counterfactuals, these approaches involve analyst discretion and interpretation of often-murky legal standards.
