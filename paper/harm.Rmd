...
abstract: |
  ...
  But these measures cannot be easily adapted to other groups, 
  including those defined by race, class, or geography.
...
# Minivation {#sec:motivation}

To introduce and motivate the concepts of individual and differential harm, we will make use of Minissouri, a stylized representations of Missouri.
Minissouri has two medium-sized cities which are not large enough to deliver a statewide Democratic majority.
Minissouri has a population of 180 and should be divided into 3 districts with population between 54 and 66.
Voters' preferences are assumed to be stable across elections, and turnout is 100%.
A schematic for this example is shown in Figure \ref{fig:minissouri}.
Each person is a square or circle, identifying some group membership or characteristic.
Each person is then shaded red or blue to correspond to Republicans and Democrats, respectively.

```{r minissouri, fig.width=6.75}
#| fig.cap: |
...
Now, consider the two plans in Figure \ref{fig:minissouri}(a)-(b).
Like in much of the US, in Minissouri Democrats are sorted asymmetrically into the cities while Republicans dominate the rural regions [@chen:rodd:13; @enos:brown:21].
A Democratic gerrymander (a) effectuates Democratic control by splitting its cities, connecting them through thin regions around the boundary of the state.
This gerrymander would elect one Republican and two Democrats, whereas the proportional map (b) would elect two Republicans and one Democrat.

If we take the Democratic gerrymander as the reference plan and the proportional plan as the counterfactual, Figure \ref{fig:minissouri}(c) highlights who is harmed.
Most of the highlighted people are the Republicans who were placed in a Democratic district in the gerrymander, but are now in a Republican district in the proportional plan.
Note that the Republicans in the north-east city are not harmed here.
In both plans, those Republicans are in a Democratic district in both the reference and the counterfactual.
There are two blue people who are harmed, as they were placed into a Republican district in the gerrymander, but now move to a Democratic district.
We can say then that 44 Republicans and 2 Democrats are individually harmed.

There's a second dimension here to individual harm.
Among the people, some are circles and some are squares.
Squares are highly correlated with the Republican choice as circles are with the Democratic choice.
Based on their vote choice, as before, we can identify who is harmed.
Now, there are circles harmed from both parties.
2 blue circles, 11 red circles, 33 red squares, and no blue squares are individually harmed.

We can use these groups, to compare how the harm done by a plan falls across those groups.
...
The Democratic gerrymander (a) harms Republicans at a much higher rate than Democrats.
...
If there is no gap, then we have *fairness*.

The partisan differential harm of the Democratic gerrymander of Minissouri is $-0.415$, where positive values indicate Republican bias.
In other words, under the Democratic gerrymander, the average Republican voter in Mininois is 41.5% more likely to be represented by a Democrat than an average Democrat is to be represented by a Republican, compared to the proportional plan.
The shape-based differential harm of the gerrymander is $-0.2$, where positive values indicate pro-square bias.
On account of being a square and not a circle, a square is 20% less likely to be represented by their candidate of choice.
...
Exchanging the role of the enacted plan and the counterfactual will not simply flip the direction of our conclusion; in general, $q$ will harm some individuals versus $\tilde q$, and $\tilde q$ will harm a distinct set of individuals versus $q$.
A "symmetrized" version of $h_i(v,q,\tilde q)$ would have to combine both of these groups individuals: those harmed as well as those who gain by electing their chosen candidate under $q$ but not under the counterfactual $\tilde q$.
In practice, voters, courts, or politicians may care more about preventing harm than maximizing gains.

Our definition of harm is limited in two key regards.
...
Ideally, we would like to be able to average individual harm over a set of future elections (under the same districting plan), or over a set of hypothetical districting plans which are consistent with districting criteria, or even over a set of individuals with some shared characteristic.

To allow for this, we replace the fixed election $v$, counterfactual districting plans $\tilde q$, and individual voter $i$ with random variables $V\in\Elecs$, $\tilde Q\in\Plans$, and $I\in\Voters$.
The random variable $V$ can be conceptualized as ranging over possible future elections---one realization might correspond to a heavily Democratic-favoring midterm year, while another might involve incumbents underperforming particularly badly.
...
At times we may wish to study on a particular election or a particular counterfactual plan; treating $V$ and $\tilde Q$ as random variables allows us to condition on particular values of each.
...
The random variable $I$ here serves as a kind of "veil of ignorance" [@rawls2004theory]; a districting plan $q$ which tends to produce a low $H(q)$ would be expected to be preferred over a plan which tended to produce a higher $H(q)$.

$H(q)$ actually contains all the relevant information about the harm created by a plan $q$.
We can "zoom in" or summarize various aspects of the plan by conditioning on values of $V$, $\tilde Q$, and $I$, or by taking expectations.
...
For algorithmic fairness, the implicit counterfactual is a perfect predictor---compared to this perfect predictor, the actual decision process harms individuals whenever it makes a false positive or false negative error.
...
Appendix \ref{app:elecmodel} contains more details about how we estimate and apply this model.
...
For example, one could operate on the logit scale, or add terms to model incumbency effects.
...
For each sample $\tilde q$ of a counterfactual districting plan, we use the election model to simulate vote shares in each district of $q$ and $\tilde q$.
We can then identify, for each precinct, how many Democratic or Republican voters were harmed.[^3]
...
[^3]: For this, we use the baseline count of Democratic or Republican votes.
    This is a slight simplification, since because the vote share changes across random draws, the number of Democratic and Republican voters does, too.
    To deal with this issue completely necessitates modeling elections at the precinct level, which requires much more data and computational expenditure.

# Connections to Current Practice for Analyzing Partisan and Racial Gerrymandering {#sec:connections}

A primary advantage of the proposed harm measures is that they are defined at the individual level, allowing for the study of redistricting far beyond racial and partisan gerrymandering.
However, the measures provide an improvement over existing analytic approaches in both of these important contexts.
...
Perhaps the new measure which has reached the most acclaim is the *efficiency gap* [@stephanopoulos2015], which measures the difference in the number of the votes wasted by each party.[^4]

[^4]: Wasted votes are those cast for a losing candidate, as well as those cast for a winning candidate beyond the required 50% to win the election.

The latter four metrics are grounded in intuition over how to reliably identify a partisan gerrymander, and not in a larger theoretical framework (in particular, they do not measure partisan symmetry [@katz2020]).
...
This is in contrast to partisan symmetry as well as the proposed harm measures, which require practitioners to take the additional step of linking observed election data to the theoretical construct by way of an election model, as discussed in Section \ref{sec:estimating}.[^5]
...
[^5]: For partisan bias, both researchers and practitioners have largely relied on simple uniform-shift models similar to the one we use in this work.

Since none of the existing measures are centered on individual voters, they offer far less granular insight than the proposed harm framework, even in the case of studying partisan gerrymandering.
For example, differential harm can be estimated at the precinct level and then plotted on a map to show not just *who* is harmed by a gerrymander, but *where* they reside.
...
Indeed, @katz2020 reference an implicit counterfactual in discussing the normative implications of partissan symmetry, writing that "[o]bviously, a process designed to be 'fair' that turns out to be substantially asymmetric [i.e., have nonzero partisan bias], if there is a fairer alternative, would not normally be regarded as a legitimate outcome."
Similar implicit counterfactuals can be thought up for the other measures.
...
@katz2020 and others recognize that a nonzero value in a measure like partisan bias can only be considered problematic "if there is a fairer alternative."
The inability of partisan bias and other existing measures to explicitly account for realistic counterfactual districting plans means that none of them may be interpreted in a vacuum.
...
The standard calculation of partisan bias, $\beta(\half)$, can give unintuitive results in more lopsided states, which, if not interpreted carefully, can lead practitioners to make incorrect conclusions about the presence and direction of a partisan gerrymander.[^6]
...
[^6]: This has been called by some the "Utah paradox" [@deford2020symmetry], though the paradoxical nature is contested as a mistake of using it for description, rather than inference [@katz2022].
...
The efficiency gap and is often highly correlated with estimates of differential partisan harm under a stochastic uniform partisan swing model (though the efficiency gap is much more discrete), as wasted votes and partisan harm are rooted in similar logic.[^7]
...
[^7]: The differences stem from the efficiency gap considering a vote to be wasted if it is in excess of the 50% threshold needed to win, while the harm framework does not penalize such votes, since every such voter is happily represented by their candidate of choice.
...
For the 2020 redistricting cycle, each party submitted a proposed plan to the chair, John Wallace Jr.
Wallace ultimately selected the Democratic plan for enactment, "simply because in the last redistricting, the map was drawn by Republicans."[^8] Here, in contrast, we evaluate the two plans on the basis of existing gerrymandering metrics as well as through the proposed harm framework.

[^8]: See Matt Friedman, "Democrats prevail in New Jersey redistricting with map that could sacrifice Malinowski," *Politico,* December 22, 2021.
...
Figure \ref{fig:nj-maps} shows the baseline Democratic vote share by precinct, along with the Democratic and Republican proposals.[^9]
...
For each of these sampled plans and the two gerrymanders, we estimate the average harm and differential partisan harm versus the simulated baseline,[^10] and calculate the expected number of Democratic seats, the efficiency gap, partisan bias, the mean-median score, and partisan declination.
The distribution of these measures is shown in Figure \ref{fig:nj-meas}.

[^10]: We choose to calculate $\Delta_{\dem-\rep}H(q)$, so that positive values correspond to a plan that favors Republicans.
...
These values are relatively directly interpretable; for example, the estimated differential harm of $-3.6%$ means that an average Democratic voter is 3.6 percentage points less likely to be misrepresented than their average Republican counter-partisan.
...
These values can be better contextualized in comparison the distribution across the sampled plans.
...
However, it is notable that while the Democratic proposal has a more extreme estimate of differential partisan harm (1.6 percentage points higher), its value of $-0.037$ is not an outlier with respect to the distribution of differential harm for a randomly sampled plan ($p=0.43$, using the same set of samples as a counterfactual), while the Republican plan is ($p=0.0002$).
...
Notably, the plans which minimize estimated total harm are also those which have near-zero estimated differential partisan harm.
Thus, at least in New Jersey, overall harm reduction is consistent with a fair map on a partisan basis.
Minimizing bias with other measures, however, could lead one astray: plans which score an efficiency gap of zero can have estimated average harm as high as 0.172 and estimated differential harm as extreme as $-0.061$, nearly twice the value of the Democratic proposal.
Plans with zero partisan bias or mean-median score likewise are estmated to have higher-than-average harm and to tend to differentially harm Republicans.
...
We next apply our proposed framework to @2022merrill, ongoing VRA litigation in which civil rights groups have argued that the existing Black majority district packs Black voters, and so a second Black opportunity-to-elect district should be drawn.[^11]

[^11]: Relevant court filings are available at <https://redistricting.lls.edu/case/milligan-v-merrill/>.
...
Council district boundaries are redrawn every ten years following the decennial census, and must be equipopulous, be compact, and preserve neighborhood and community boundaries.[^12]
...
To solve both of these problems, we develop a *latent factor ecological inference model*, which simultaneously estimates (a) turnout by precinct and race, (b) candidate positions in a latent two-dimensional political space,[^13] and (c) voter preferences in this space by precinct and race, using precinct-level demographic data and vote totals from the March 2020 presidential primary and September 2021 mayoral preliminary elections.
...
[^13]: Exploratory analyses found that three latent dimensions did not provide a substantially better fit to the data.
...
Comparing the enacted plan to the the simulations shows who is harmed by the current plan versus a race-neutral baseline; the reverse comparison shows who would be harmed by a race-neutral baseline in comparison to the current plan.
For this reverse comparison, along the first dimension, we estimate that a race-neutral baseline would differentially harm Black voters at a rate of 3.1% compared to the enacted plan
Along the second dimension, the differential harm to Black voters would be 1.8%.

Taken together, these findings suggest that despite the dominance of the Democratic party in city politics, and the shared preferences between certain groups of Black and White voters, race-aware districting still serves an important role in empowering Black voters across the city to elect their chosen candidates, and that a switch to race-neutral boundaries would have a material impact on Black voting power.
...
For example, the Washington's 7th congressional district receives one random effect from 2002-2010 and a different random effect from 2012-2020, since new district boundaries were enacted in the 2010-2012 period.
