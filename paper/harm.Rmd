...
The core of the voter choice model is the following specification for $\pi_{jk}$, the preference of precinct $j$ for candidate $k$ on the logit scale:[^13] \begin{align*}
    ...
    \pref_{j}^\top(\overbrace{P_j}^{\mathclap{\substack{\text{diagonal matrix of}\\\text{precinct racial proportions}}}}
    \underbrace{\turn_j}_{\mathclap{\substack{\text{turnout rate parameter}\\\text{from turnout model}}}}
    )^\top\vb L_k.
\end{align*} Here, $\pref_j$ is a matrix of preferences by race and latent dimension within precinct $j$, so $\pref_{j}^\top(P_j\turn_j)^\top$ is a vector with elements containing the preferences of the precinct's voters in each latent dimension, after marginalizing out race.
The vector $\vb L_k$ contains the loading of candidate $k$ onto the latent space: When $\vb L_k$ and $\pref_{j}$ are nearby in latent space, then their product will be large, and the precinct's support for the candidate, $\pi_{jk}$, will be large.
...
The second dimension appears primarily to capture racial differences in support---as the factor value maps in Figure \ref{fig:boston-factors} show, minority-majority and White areas of the city score opposite values on the second dimension.
...
The logit candidate preference itself is modeled as \begin{align*}
    \pi_{jk} &= \beta^{(s)}_k + \alpha_k
        \pref_{j}^\top(\turn_j\circ \vb p_j)^\top\mathrm{diag}(\vb*\rho)\vb L_k  \\
