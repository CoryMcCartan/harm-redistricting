...
Doing so both unifies our approach to gerrymandering across contexts and also reflects a core goal of representative democracy: that elected officials represent the preferences of the voting public to the greatest extent possible [@brunell2010; other cites on why districts are good. Federalist No. 52?].
...
A more detailed discussion of the connections between the proposed framework existing practices for analyzing racial and partisan gerrymandering may be found in Section \ref{sec:connections}.
...
The modern analysis of racial gerrymandering and vote diluation are inseparable from the Voting Rights Act of 1965 (VRA) and the 14th Amendment.
...
In contrast, the partisan bias and mean-median measures incorrectly classify the Republican gerrymander as a fair plan and a Democratic-leaning outlier, respectively.

Next, we consider the distribution of these statistics across the sampled plans, but this doesn't help much, either.
The expected number of Democratic seats naturally identifies the gerrymanders (two-sided $p=0.0002$) and the direction of bias, and as discussed above this measure is extremely correlated with differential partisan harm.
...
Democrats do win 8.47 seats on average across the simulations, which is more than the 6.79 seats that would be expected under a completely proportional outcome.
The key benefit here is that differential harm does not push for localized gerrymanders to overcome the geographic imbalance in partisanship.
...
Appendix \ref{app:corr} calculates these correlations across eight states.
The most consistent correlation is that between our estimate of differential partisan harm and the expected number of seats won by each party, which is usually around 0.98, and always above 0.66 in the eight states we study.
...
One could weight voter harm by the distance in latent space between each voter's preferred candidate and the elected candidate, thus dropping the implicit assumption that all candidates the voter did not vote for are exchangeable.
...
We offer a new framework which unifies varied strains of thoughts from common law standing to the logic of counterfactuals for causal inference.
By using counterfactual redistricting plans, we reset the focus on voters who could be better represented.
This imbues the long-standing understandings of standing into the quantity of interest.
Once measured, this appreciably increases the accuracy in identifying partisan gerrymanders---both in sign and in existence.
It goes beyond partisanship, too, allowing us to identify differential harms in other groups, such as by race.
Yet, by centering the framework on voters and vote choice, we can calculate the harm to individuals beyond their group memberships.
For any estimated relationship between the voters and their vote choice, such as their within-party ideology, we can estimate the harm to each individual.

This flexibility stems from the development of a novel standard of fairness in redistricting.
Moving away from partisan symmetry, we look to the rate at which voters are deprived of their ability to elect their chosen candidate.
We offer measurement strategies for this quantity, building largely out of advances in simulation methodologies.
Taken together, this gives one unified and feasible framework for studying redistricting.

Yet, this is surely not the end of the menagerie.
Flexibility comes at a cost, the largest being the specification of a counterfactual.
While a representative sample of legally feasible plans may be a reasonable starting point, full inclusion of discretionary choices can be subjective.
When subjective choices are not represented consistently, the analysis may differ.
Further, modelling future elections is simply hard.
While incorporating model free estimates based on past results is common, there is no assurance in future validity.
When we do write a model, we often rely on very powerful simplifying assumptions, such as uniform swing models.
Computationally, bringing all of these parts together remains difficult, as you need the ability to compute everything for each counterfactual plan.
Each of these limitations present opportunities for future study.

Redistricting is difficult to study.
At its core, it is a high dimensional problem that must balance normative, legal, and administrative concerns to allow citizens to exercise their rights.
We don't believe that this framework fixes the dimensionality problem.
But, we do offer a modular and flexible framework to allow researchers to study more dimensions together.
This framework allows people to tinker with their basic assumptions and decompose how various pieces contribute to observed patterns and impacts.
Researchers can and should push our framework further to apply it better to multi-candidate elections, more realistic models, and beyond district-based electoral systems.


Discussion outline suggestion:

-   Recap goal and advantages.
    Unified framework for studying redistricting.
    Considers counterfactuals.
    Centers vote choice, and separates vote choice from group membership.

-   Recap strengths.
    New normative standard.
    Separation of target and estimation.
    Flexible framework.
    Application to local elections and places without strong parties.
    
-   Discuss limitations.

    -   Definition of counterfactual subjective & has a big impact.
        Some kind of bounds analysis where we look at best/worst case counterfactuals?

    -   Limited for multi-candidate settings where voters have ordinal preferences
    Even given a model for two candidates, extending this approach to more complex elections, say multi-candidate election with ordinal preferences, is not yet done.

    -   Election models limited

    -   Simplyfying estimating assumptions like uniform swing

    -   Computational complexity, especially versus status quo measures which are cheap to compute.

-   Final ¶: This stuff is complex.
    Best we can do is to have a modular & flexible framework where people can tinker with assumptions and decompose how various pieces contribute to observed patterns and impacts.
    Researchers can and should push our framework further to apply it better to multi-candidate elections, more realistic models, and beyond district-based electoral systems.
