...
We will say that a plan $q$ is *fair* in comparison to a counterfactual $\tilde Q$ if $\E[H(q)\mid I\in G_k]$ is constant across groups $G_k\in\Groups$.
...
Appendix \ref{app:formulas} contains the specific formulas used to compute $H(q)$, and $\Delta H(q)$ from a set of past election returns and a sample of districting plans from a redistricting sampling algorithm.
...
For each of these sampled plans and the two gerrymanders, we calculate the utility, harm, and fairness measures, as well as the expected number of Democratic seats, the efficiency gap, partisan bias, and the mean-median score.[^1]
The distribution of these measures is shown in Figure \ref{fig:nj-pairs}.

[^1]: We include partisan bias and mean-median in these figures as it is common practice, though it may have theoretical issues when used for a delegation rather than a legislature[@katz21].
...
$$ which is the inverse CDF of a Laplace distribution with rate 2.[^2]
...
[^2]: A rate of 2 is chosen so that the slope of the link function at $\half$ is exactly 1, meaning that transformed vote shares will be on approximately the same scale as the original shares.
