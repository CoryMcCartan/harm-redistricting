...
The Efficiency Gap identifies that the enacted map favors Democrats, whereas both Partisan Bias and Mean-Median indicate a pro-Republican bias under reconstituted recent elections.^[This is according to [PlanScore](https://planscore.campaignlegal.org/massachusetts/#!2020-plan-ushouse-eg), run by the Campaign Legal Center.]

Yet, these scores lack a counterfactual.
Functionally, neither Republicans nor Democrats could do better or worse under this plan than any other plan---a counterfactual plan.
By relying on counterfactual plans, we can then identify the cases where individuals are not represented by their preferred candidate or party, as an alternative to existing partisan bias mesaures.
In places like Minichusetts, as no plan can be drawn which improves the partisan representation of Republican voters, under a counterfactual-driven approach, then there is no harm or bias.
...
This allows for more variation in the expected partisanship of the districts drawn.
...
This pattern allows us to build out the intuition behind our proposed measure.
Each voter belongs to a group, mostly simply a party.
For this, we offer that a voter is harmed if they are not represented by their party of choice, but have a counterfactual where they would be represented by their party of choice.

Consider the example in Figure \ref{fig:harm-mininois}.
A Democratic gerrymander in Mininois effectuates Democratic control by splitting its city into with the blades of a pinwheel, reaching out into the more Republican and less population-dense remainder of the state.
We see that the Democratic gerrymander would likely elect zero Republicans and three Democrats, whereas the proportional map would likely elect two Democrats and one Republican.
Figure \ref{fig:harm-mininois}, Panel C then illustrates th total harm by district, comparing the gerrymander to the proportional plan. 
Republican voters who are forced into a Democratic district under the gerrymander, but would not be under a proportional plan are deemed harmed.
The Republicans who stay in Democratic districts in the proportional plan are not harmed.
