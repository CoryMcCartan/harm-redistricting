...
<!--# Yet, this suffers from a core inflexibility: it does not allow for variation in districting. Districting is the fundamental question of district-based democracies. For whom can you vote? Who do you vote with? Our proposed framework rectifies this inflexibility, by shifting to a focus on individual representation. The natural measurement approach to this directly incorporates alternatives under the logic of a counterfactual. -->
...
Differential partisan harm correctly identifies each gerrymander as such, with a large negative value for the Democratic gerrymander and a large positive value for the Republican gerrymander.
...
Next, we consider the distribution of these statistics across the sampled plans, but this doesn't help much, either.
...
We next apply our proposed framework to @2022merrill, ongoing VRA litigation in which civil rights have argued that the existing Black majority district packs Black voters, and a second Black opportunity-to-elect district should be drawn.

As discussed in Section \ref{subsec:racial}, plaintiffs had to satisfy the first *Gingles* criterion by submitting one or more remedial maps demonstrating that a new Black opportunity-to-elect plan could be drawn while meeting all other redistricting requirements.
...
These plans form a natural counterfactual $\tilde Q$ to use in analyzing the challenged districting plan (Figure \ref{fig:al-maps}(d)), especially since the federal district court ruled in favor of the plaintiffs and found that their remedial plans satisfied the first *Gingles* criterion.[^8]
...
Unsurprisingly, voting is highly racially polarized outside of the cities of Birmingham and Montgomery.

With these estimates in hand, we can calculate the average number of voters harmed in each precinct and racial group.
Figure \ref{fig:al-maps}(c) shows the geographic distribution of harm.
...
It is precisely these areas that are excluded from the Black-majority district under the challenged plan, but are included in a Black-majority district in the average remedial plan.
...
Across the state, the average harm is 0.075, which translates to around 151,000 voters.
Of these harmed voters, around 128,000 are Black, while 23,000 are White.
Similarly, 140,000 are Democratic, while only 11,000 are Republican.
Of course, the total number of each of these groups of voters is very different.
Normalizing by group sizes, the expected harm for a Black Democrat is 0.23, for a Black Republican is 0.026, for a White Democrat is 0.084, and for a White Republican is 0.0082.
Thus while the challenged plan differentially harms both Black voters and Democratic voters, it *especially* harms Black Democrats, who have a differential harm of 0.149 versus White Democrats and a differential harm of 0.207 versus Black Republicans.
...
Second, as in Alabama, we aim to examine the racial impact of the district system, and thus need to estimate these voting preferences by race.
...
Specifically, we draw voter choices from the posterior predictive distribution of our model, setting the candidates' loadings to --1 and 1 on the chosen dimension and 0 on the other dimension.
...
For the first scenario, where council races are contested along the ideological axis, we find that the enacted plan is quite fair, differentially harming White voters at a rate of only 0.0079 compared to the race-neutral baseline.
...
Along the first dimension, a race-neutral baseline would differentially harm Black voters at a rate of 0.031 compared to the enacted plan, while along the second dimension, the differential harm would be 0.018.
...
We model the observed vote counts as \begin{align*}
...
\end{align*} where the $\vb*\beta^{(t)}$ and $\vb*\beta^{(s)}$ model election-specific logit shifts, $\Binlog$ indicates the Binomial distribution with the probability paramter specified on the logit scale, and $\circ$ indicates the elementwise (Hadamard) product.
We give $\beta^{(t)}$ an i.i.d. $\Norm(0, 0.6^2)$ prior,and $\beta^{(s)}$ an i.i.d. $\Norm(0, 0.4^2)$ prior, and $\mu^{(t)}$ an i.i.d. $\Norm(0, 1.5^2)$ prior, and $\mu^{(s)}$ an i.i.d. $\Norm(0, 1.0^2)$ prior.
...
We model the observed vote counts as \begin{align*}
    ...
    \logit(\turn_j) \mid \mu^{(t)}, \Sigma^{(t)}
        &\sim \Norm(\mu^{(t)}, \Sigma^{(t)}) \\
    \logit(s_{jk}) \mid p_{jk}, \eps &\sim \Norm(p_{jk}, \frac{\eps}{1+\votes_{jv_k}}) \\ 
    p_{jk} &= \beta^{(s)}_k + \alpha_k
        \pref_{j}^\top(\turn_j\circ \vb p_j)\mathrm{diag}(\vb*\gamma)\vb L_k  \\
...
The matrix $\mathrm{diag}(\vb*\gamma)$ captures the relative scale or importance of each dimension to the overall outcomes.

Because of the latent factor model, we do not require that vote shares by race and precinct, when tallied by race, necessary equal the observed vote share in the precinct.
...
We give $\beta^{(t)}$ an i.i.d. $\Norm(0, 0.5^2)$ prior,and $\beta^{(s)}$ an i.i.d. $\Norm(0, 4.0^2)$ prior, and $\logit(\mu^{(t)})$ an i.i.d. $\Beta(10, 30)$ prior.
...
The scale parameters $\vb*\gamma$ have i.i.d. $\Gam(1.5, 1.5/4.0)$ priors, the fudge factors $\vb*\alpha$ have i.i.d. $\Gam(5.0, 5.0)$ priors, and $\eps$ has an $\Expo(1/4.0)$ prior.
...
This translated to a prior of $\Norm((-1, 0), (0.1^2, 0.25^2))$ for Sanders,
$\Norm((1, 0), (1.0^2, 0.5^2))$ for Biden, and $\Norm((-0.25, -1), (0.5^2, 2^2))$ for Janey.
...
We fix the model's loading matrix $\vb L$ to the the posterior mean of their estimated loadings.
