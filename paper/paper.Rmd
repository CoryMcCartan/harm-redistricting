...
A natural extension of studying the average harm to voters is to examine how harm is distributed across certain groups of voters, defined by race, party, or other attributes of interest.
Let $\Groups=\{G_1,G_2,\dots\}$ be a set of mutually exclusive groups of voters, so that for each voter $i$, we have $i\in G_k$ for some group $G_k$.

Then we say that a plan $Q$ is *fair* in comparison to counterfactual $\tilde Q$ if $\E[H\mid I\in G_k]$ is constant across groups $G_k\in\Groups$.

To quantify deviations from fairness, we introduce of a measure of \textit{differential harm}, defined as $$
  \Delta_{G_k-G_l}H(q) \dfeq \E[H\mid I\in G_k, Q=q] - \E[H\mid I\in G_l, Q=q],
$$ for two groups $G_k$ and $G_l$.
So $\Delta_{G_k-G_l}H(q)=0$ for all $G_k,G_l\in\Groups$ if and only if $q$ is fair.
<!--
...
Despite being derived from a local harm measure, $\Delta H(Q)$ is in fact closely related to the overall number of seats won by each group, as Proposition \ref{prop:fairness} describes below.
Yet unlike a simple measure of seats, $\Delta H(Q)$ is shifted to an interpretable scale, where a value of zero corresponds to fairness.
...
Suppose $\Groups=\{G_1,G_2\}$. If turnout is equal across districts, then for a specific districting plan $q\in\Plans$,
\begin{align*}
    \Delta_{G_1-G_2} H(q) = \E[\seats(G_1, V, Q)\mid Q=q] - \E[\seats(G_1, V, \tilde Q)].
\end{align*}
...
The measures $H$ and $\Delta H(q)$ defined above require specification of a probability distribution on elections and districting plan.
...
As far as estimating measures like $H$ and $\Delta H(q)$, we would ideally use a relatively simple model that captures the primary drivers of election outcomes.
...
Appendix \ref{app:formulas} contains the specific formulas used to compute $H$, and $\Delta H(q)$ from a set of past election returns and a sample of districting plans from a redistricting sampling algorithm.
