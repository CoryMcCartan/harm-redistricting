...
```{r boston-map, fig.cap="Racial composition of the city of Boston (left), and boundaries of the 2010 city coucil districts (right). Neighborhood boundaries are overlaid in black on the left map and in white on the right map.", fig.width=6.75, fig.height=3.375}
...
In practice, districting plans are built from discrete geographic units (often precincts), and election returns are reported at this level as well.[^5]
...
[^5]: When district boundaries split precincts, practitioners generally impute the returns in each component of the split precinct by population-weighted interpolation.
    Population information comes from Census blocks, which are practically never split.

## Proportionate Uniform Shift Details

The proportionate uniform shift model assumes that from one election to another, a constant fraction of *voters of one party* in each precinct switch to the other party, and that per-precinct turnout is constant across elections.[^6]
...
[^6]: Everything carries through if one allows for shifts in the overall turnout level across elections, as long as (a) the relative turnout across precincts is fixed, and (b) turnout shifts are independent of shifts in party preferences.
...
$$ which is the inverse CDF of a Laplace distribution with rate 2.[^7]
...
[^7]: A rate of 2 is chosen so that the slope of the link function at $\half$ is exactly 1, meaning that transformed vote shares will be on approximately the same scale as the original shares.
