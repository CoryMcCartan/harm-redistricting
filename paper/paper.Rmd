...
```{r mini-schematic}
#| fig.cap: |
#|      Schematic maps of the three running examples,
...
Estimating these quantities from available data is another matter, of course, and requires additional assumptions, as we describe in the next section.
First, however, we note two interesting consequences of our definition of differential harm.

In the partisan case, with $\Groups=\{\dem,\rep\}$, differential harm is in fact closely related to the overall number of seats won by each party, as Proposition \ref{prop:fairness} describes below.
...
The canonical framework for considering partisan fairness is partisan symmetry [@king1987democratic]. Partisan symmetry implies that the seats, $s$ won by a party with voteshare $v$ would be satisfied if $s(v) = 1 - s(1 - v)$. Often, the partisan symmetry framework has been applied via partisan bias, which measures the seat deviation from symmetry [@tufte1973pb; @katz2020].
Partisan bias, denoted by $\beta(s)$ is the number of seats that would need to be reallocated to reach proportionality for a given statewide vote share $s$ (usually $\half$).
To compute partisan bias, researchers have largely relied on assumptions about how the voteshares would swing to reach the alternative voteshares.
...
Perhaps the new measure which has reached the most acclaim is the *efficiency gap* [@stephanopoulos2015].
Referred to by Chief Justice Roberts as "sociological gobbledygook" in Gill v. Whitford, the efficiency gap looks to measure the votes wasted by the parties and compare the net wasted votes.
Notably, while the efficiency gap was intended to measure partisan symmetry, it does not do so [@katz2020].
...
The idea of partisan symmetry is rooted in assessing how parties would have performed under counterfactual voteshares.
...
Consider the major issues that have been raised over existing measures.
Partisan bias ($\beta(\half)$) can give unintuitive results in more lopsided states, which, if not interpreted carefully, can lead practitioners to make incorrect conclusions about the presence and direction of a partisan gerrymander.
This has been called by some the Utah paradox [@deford2020symmetry], though the paradoxical nature is contested as a mistake of using it for description, rather than inference [@katz2022]. A number of scholars have criticized the efficiency gap for (1) penalizing proportionality, (2) rewarding 3-to-1 districts and switching sign for more lopsided districts, and (3) being discrete and therefore sensitive to small changes in district vote shares [@bernstein2017egap; @tamcho2017egap; @deford2020symmetry].
These stem from the omission of the alternatives.

By looking to alternatives, we ground the idea of zero in a balance of the harms, rather than a purely theoretical way.
This resolves the directionality problem common to other measures.
The use of a counterfactual in measuring the quantity of interest grounds the bias in the rules of what districts can be drawn.
This allows us to draw key strength from the impetus of the redistricting simulation literature.
Yet, the simplicity of the framework does not restrict only to the use of simulation methods.
Further, this framework does not need to rely on ideas of proportionality, which has no legal role in American democracy.
Nor does it suffer from discreteness in ensembles, as now both voteshares and districts lines shift.

Despite these differences, there are also key empirical similarities in many cases.
The efficiency gap and $\Delta H(q)$ are often quite well correlated under competitive states, as wasted votes and partisan harm are rooted in similar logics.
Similarly, in competitive states, where the baseline election has a statewide vote share near 50%, $\beta(\half)$ is highly correlated with $\E[\seats(\dem, V, Q)\mid Q=q]$ and thus $\Delta H(q)$ as well.
Where our framework rises above these though is especially in non-competitive states, where insurmountable advantages are rendered as zeroes and do not contribute to harm.

# Application to Partisan Gerrymandering in New Jersey
...
We next turn to litigation in Alabama, Merill v. Milligan, where civil rights groups sued Alabama arguing that a second Black opportunity-to-elect district should have been drawn. For litigation under Section 2 of the Voting Rights Act of 1965 (VRA), plaintiffs submit one or more remedial maps in order to meet the preconditions set under Thornburg v. Gingles (478 US 30) in 1986. Remedial maps demonstrate that a new plan could be drawn to comply with the VRA. We use four remedial plans submitted by plaintiff's expert, Moon Duchin, which each create a moderately compact, second Black opportunity district while complying with other state redistricting criteria. We opt to use remedial plans, as they offer a relevant alternative set of plans ($\tilde{Q}$) to which courts already look when establishing liability.

Figure \ref{fig:al-maps} displays the results of this process. 
Panels (a) and (b) of \ref{fig:al-maps} display the partisanship and Black voting age percentages of Alabama.
Unsuprisingly, Black population and Democratic voteshare are highly correlated.
The remedial plans create a Black opportunity district by connecting Mobile to Montgomery, via the Black Belt region.
Panel (c) of \ref{fig:al-maps} highlights the total harm from the enacted Alabama plan against the remedial plans.
...
For example, in the north of Alabama, near Huntville, despite a larger number of Democratic voters in a Republican district, none of them are harmed here.
...
There is also a pocket of harmed voters in Tuskaloosa, above the left shoulder of district 7.
...
After normalizing by group sizes, this gives a substantial  differential harm of 20.9%.
...
Thus, once normalized by group size, the differential racial harm is 23.7%, indicating that Black voters are harmed substantially more than white voters.
...
Council district boundaries are redrawn every ten years following the decennial census, and must be equipopulous, be compact, and preserve neighborhood and community boundaries.[^4]
...
To solve both of these problems, we develop a *latent factor ecological inference model*, which simultaneously estimates (a) turnout by precinct and race, (b) candidate positions in a latent two-dimensional political space,[^5] and (c) voter preferences in this space by precinct and race, using precinct-level demographic data and vote totals from the March 2020 presidential primary and September 2021 mayoral preliminary elections.
...
[^5]: Exploratory analyses found that three latent dimensions did not provide a substantially better fit to the data.
...
In practice, districting plans are built from discrete geographic units (often precincts), and election returns are reported at this level as well.[^6]
...
[^6]: When district boundaries split precincts, practitioners generally impute the returns in each component of the split precinct by population-weighted interpolation.
    Population information comes from Census blocks, which are practically never split.

## Proportionate Uniform Shift Details

The proportionate uniform shift model assumes that from one election to another, a constant fraction of *voters of one party* in each precinct switch to the other party, and that per-precinct turnout is constant across elections.[^7]
...
[^7]: Everything carries through if one allows for shifts in the overall turnout level across elections, as long as (a) the relative turnout across precincts is fixed, and (b) turnout shifts are independent of shifts in party preferences.
...
$$ which is the inverse CDF of a Laplace distribution with rate 2.[^8]
...
[^8]: A rate of 2 is chosen so that the slope of the link function at $\half$ is exactly 1, meaning that transformed vote shares will be on approximately the same scale as the original shares.
