...
Schematics for these examples are shown in Figure \ref{fig:mini-schematic}.
...
Section \ref{sec:indiv-harm} builds a measure of individual harm from this notion of utility.
Section \ref{sec:empirical} applies this measure to a real-world example.
...
Let $S=\{s_p:p\in P\}$ be a set of party popular vote shares, and $f:[0,1]\bijto[0,1]$ be a rule for assigning each party a share of seats according to their share of the popular vote.
Define the total utility for this rule as \[
    U_e^{(G)}(f) = \sum_{v\in V} h\qty(f\qty(s_{\vote_e(v)})).
\] 
Then if $h$ is convex, $U_e^{(G)}$ is maximized for $f(x)=\ind\{x\ge \max S\}$, and if $h$ is strictly concave, the maximizer of $U_e^{(G)}$ will be a strictly increasing function.
