...
To do so, we use an ecological inference model which jointly estimates turnout and vote choice by race and precinct.
The model and its outputs are detailed in Appendix \ref{app:alei}.
...
Unlike in a traditional EI/*Gingles* analysis, there is no need to identify a particular party as the statewide "choice" of the minority racial group, nor is there any cause for concern because of lower racial voting polarization in urban areas.
...
Our ecological inference model used to produce the estimates is broadly similar to that of @king1997solution. However, it jointly estimates both turnout and Democratic support across all seven elections, rather than estimating each quantity separately for each election.
...
#| fig.cap: |
#|      EI model estimates of precinct-level turnout by race (a) 
...
We model the observed vote counts as
\begin{align*}
...
\end{align*}
where the $\vb*\beta^{(t)}$ and $\vb*\beta^{(s)}$ model election-specific logit shifts, $\Binlog$ indicates the Binomial distribution with the probability paramter specified on the logit scale, and $\circ$ indicates the elementwise (Hadamard) product.
We give $\beta^{(t)}$ an i.i.d. $\Norm(0, 0.6^2)$ prior,and $\beta^{(s)}$ an i.i.d. $\Norm(0, 0.4^2)$ prior,
and $\mu^{(t)}$ an i.i.d. $\Norm(0, 1.5^2)$ prior, and $\mu^{(s)}$ an i.i.d. $\Norm(0, 1.0^2)$ prior.
