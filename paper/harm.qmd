...
Appendix @app:elecmodel contains more details on how we estimate and apply this model.
...
Appendix @app:corr records these correlations across simulated plans in eight states.
...
In Appendix @app:alsampling, we repeat the analysis performed here on a different counterfactual consisting of plans sampled with a redistricting simulation algorithm; the results are broadly similar.
...
The model, detailed in Appendix @app:alei, jointly estimates two turnout and vote choice by race and precinct across seven statewide elections using a setup following @king1997solution.
...
Figure @fig-boston-factors shows the key outputs from this model, and Appendix @app:lfei explains the model in more detail.
...
As Appendix @app:lfei discusses, these assumptions are almost certainly not *strictly* met.
...
# Additional Sampling Analysis for Alabama {#app:alsampling}
...
Our ecological inference model used to produce the estimates expands on the model in Appendix @app:alei, jointly estimating both turnout and vote choice in the two elections.
