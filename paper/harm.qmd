...
This literature is characterized by a focus on identifying the voting preferences of racial groups through techniques such as ecological inference [@oloughlin1982id; @grofman1991identifying; @king1997solution; @greiner2006ecological].
...
By focusing only on party effects, existing tools overlook how partisan lines also have an effect of relative voting power by race, wealth, age, and urbanicity.
...
The harm framework combines two key ideas, further motivated in Section @sec-motivation.
...
Many factors influence the performance and qualities of a districting plan.
Without a well-defined comparison, it is impossible to differentiate whether certain aspects of the system, like its partisan performance, are consequences of laws and political geography or result from improper manipulation of the system for political gain [@chen:rodd:13].
For example, in Massachusetts, it is currently impossible to draw even one reliable Republican seat, despite 32% of voters being Republican [@duch:18].
The 9-0 Democratic composition reflects the homogeneity of the state's political geography, not partisan gerrymandering.
...
Many metrics rely on *implicit* counterfactuals, which may be irrelevant or infeasible.
...
The combination of individual voter choices and counterfactual districting plans defines *individual harm*: informally, a voter is harmed if they would be represented by their candidate of choice under a counterfactual plan.
By examining how harm varies across partisan, racial, or ideological lines, we link it back to traditional gerrymandering.
Section @sec-motivation gives a simple example Section @sec-indiv-harm formalizes it.
...
This distinction is also missed by many critics of the efficiency gap [@veomett2018efficiency; @deford2020symmetry].
The challenge of estimating harm and differential harm from observed data is discussed in Section @sec-estimating.
...
Our proposed harm measures improve on exististing racial and partisan gerrymandering analytical approaches.
The harm approach corresponds well to measuring the Gingles criteria, can directly use data from ecological inference, and handles complex cases like multi-minority coalitions [@stephanopoulos2014south]
Further, we explicitly model the counterfactuals and their feasibility, allowing us to improve on existing issues with other measures such as partisan bias [@king1987democratic], efficiency gap [@stephanopoulos2015], and declination [@warrington18].
A more detailed discussion of the connections between the proposed framework and existing practices for analyzing racial and partisan gerrymandering may be found in Section @sec-connections.

Harm and differential harm offer practical value across diverse contexts.
Section @sec-nj analyzes New Jersey congressional districts, showing how traditional metrics can misidentify both the magnitude and direction of partisan gerrymanders.
Section @sec-alabama uses differential harm to disentangle racial and partisan effects in Alabama's districting.
Section @sec-boston applies the framework to local redistricting, quantifying the benefits of race-aware boundaries for Black voters in Boston.
Section @sec-discussion concludes.

# Motivation {#sec-motivation}

Consider *Minissouri*, a stylized representation of Missouri which is shown in Figure @fig-minissouri.
Its total population is 180, and it is divided into three districts with population between 54 and 66.
Minissouri has two cities and a substantial rural population with a partisan breakdown of 55.6% R to 44.4% D.
...
Suppose the redistricting plan in Figure @fig-minissouri(a) has been enacted.
...
Intuitively, plan (a) unfairly packs Republicans into a rural district.
We can identify which voters are harmed by the enactment of plan (a) over the alternative (b).
These voters are highlighted in Figure @fig-minissouri(c).
...
This means square voters are 20 percentage points more likely to be harmed than circle voters.
While this number cannot establish whether the drawers of plan (a) intended to harm square voters, or aimed to maximize Democratic representation, it quantifies the impact of plan (a) on these voters versus the alternative plan (b), on the same scale as differential partisan harm.

# Individual and Differential Harm {#sec-indiv-harm}

We formalize and generalize the concepts in Section @sec-motivation.
...
The assumptions we make on these relationships will be made more explicit in Section @sec-estimating, where we discuss how to estimate our measures in practice.

## Harm {#sec-harm}

The key intuition from Section @sec-motivation is that to be harmed, a voter must have a better outcome in the counterfactual world where a different districting plan was in place.
...
Section @sec-boston further discusses the implications of this particular specification in the context of primary elections.
...
A particular specification of the distribution of $V$ and $\tilde Q$ is required only when *estimating* our harm measures in a particular application, a topic we return to in Section @sec-estimating.
...
## Differential Harm and Fairness {#sec-diff-harm}
...
Estimating these quantities from available data is another matter, of course, and requires additional assumptions, as we describe in @sec-methods.
...
Figure @fig-nj-maps shows the baseline Democratic vote share by precinct, along with the Democratic and Republican proposals.[^8]
...
One such plan is shown in Figure @fig-nj-maps(d).
...
The distribution of these measures is shown in Figure @fig-nj-meas.
...
Figure @fig-nj-pairs in the appendix expands on Figure @fig-nj-meas to also show the correlations between these various measures.
...
As discussed in Section @sec-racial, plaintiffs had to satisfy the first *Gingles* criterion by submitting one or more remedial maps demonstrating that a new Black opportunity-to-elect district could be drawn while meeting all other redistricting requirements.
In *Merrill,* the plaintiff's expert, Moon Duchin, submitted four such remedial plans, one of which is shown in Figure @fig-al-maps(e).
These plans form a natural counterfactual $\tilde Q$ to use in analyzing the challenged districting plan (Figure @fig-al-maps(d)), especially since the federal district court ruled in favor of the plaintiffs and found that their remedial plans satisfied the first *Gingles* criterion.
...
The model, detailed in Appendix @sec-app-alei, jointly estimates two turnout and vote choice by race and precinct across seven statewide elections using a setup following @king1997solution.
...
Figure @fig-al-maps(c) shows the estimated geographic distribution of harm.
Even without the ecological estimates, it is immediately clear, with reference to the partisan and racial patterns shown in Figures @fig-al-maps(a) and (b), that harm is concentrated among Black and Democratic voters in Birmingham, Montgomery, and the southern end of the Black Belt region.
...
This intuition is confirmed by a closer analysis, summarized in Figure @fig-al-maps(f).
...
Figure @fig-boston-map shows the racial composition of the city as well as the boundaries of the 2010 city council districts and the city's official neighborhoods.
...
Figure @fig-boston-factors shows the key outputs from this model, and Appendix @sec-app-lfei explains the model in more detail.
...
The second dimension appears to capture racial differences in support---as the factor value maps in Figure @fig-boston-factors show, minority-majority and White areas of the city take opposite values on the second dimension.
...
As Appendix @sec-app-lfei discusses, these assumptions are almost certainly not *strictly* met.
...
As Figure @fig-boston-factors shows, primary voting preferences in Boston are polarized by race, especially for the second latent dimension.
...
As Section @sec-nj--@sec-boston showed, this opens up space for more nuanced analyses that can extend to the local level as well.
...
And as discussed in Section @sec-harm, multi-candidate elections where voters have strong ordinal preferences provide further challenges that the proposed framework does not fully address.
...
In Section @sec-boston we consider a spatial-racial model for primary elections with multiple candidates, to illustrate the application of our framework to these more complicated cases.
...
Appendix @sec-app-elecmodel contains more details on how we estimate and apply this model.
...
This is in contrast to partisan symmetry as well as the proposed harm measures, which require practitioners to take the additional step of linking observed election data to the theoretical construct by way of an election model, as discussed in Section @sec-estimating.[^4]
...
Correlations, of course, are not everything: it is the universal scale and centering of our harm measure which makes it more useful in evaluating partisan gerrymanders, as we demonstrate in Section @sec-nj.
...
\setcounter{table}{0}
```
# Two-party Election Model {#sec-app-elecmodel}
...
Figure @fig-nj-pairs expands on Figure @fig-nj-meas in the main text with pairwise scatter plots and correlation summaries.
...
Table @tab:corr below reports the correlation between differential harm and the partisan gerrymandering metrics used in Section @sec-nj for eight states which cover a wide range of partisan leans and number of seats:
...
In particular, the New Jersey simulations are not the same as those used in Section @sec-nj.
...
\end{table}
```
# Ecological Inference Model for Alabama {#sec-app-alei}
...
Posterior medians of $\turn_{jr}$ and $\supp_{jr}$ are plotted in Figure @fig-alabama-ei.
...
Here, we perform the same analysis as in Section @sec-alabama, but using a randomly sampled set of plans as a counterfactual, rather than the four plans used in litigation.
...
Figure @fig-alabama-harm-sample summarizes the results, which are mostly similar to the results in in Section @sec-alabama.
...
# Latent Factor Ecological Inference Model for Boston Primary Elections {#sec-app-lfei}
...
Our ecological inference model used to produce the estimates expands on the model in Appendix @sec-app-alei, jointly estimating both turnout and vote choice in the two elections.
...
The estimated loading matrix $\vb L$ is visualized on the left of Figure @fig-boston-factors, and $F$ is visualized for Black and White voters on the right.
...
The model's predictions (posterior medians) for the turnout and vote share in each precinct are shown in Figure @fig-boston-gen-pred.
