...
format:
    cmc-article-pdf:
        ...
        natbiboptions: "super,numbers,compress"
...
Some existing districting rely on *implicit* counterfactuals, which may be irrelevant or infeasible, as we discuss in Supplementary Information A.
...
The harm measure also captures or generalizes the three *Gingles* preconditions for vote dilution litigation under the Voting Rights Act of 1965, as discussed further in Supplementary Information A.
...
We discuss this difference more in Supplementary Information A, and compare the two measures empirically in @sec-nj.
...
**Supplementary Information B and C** contain further comparisons between gerrymandering metrics.
...
To do so, we generate 10,000 districting plans without any partisan data, using the SMC algorithm of [@mccartan2020; @redist] setting constraints to favor compact plans that respect county lines.
...
The raw metrics, and these two counterintuitive results, can be better contextualized in comparison to the distribution across the sampled plans, something we discuss in Supplementary Information B.
That Supplementary Information also compares the correlations between the various metrics in the simulated sample.
We find strong correlations of differential harm with the number of seats won by each party (correlation of 0.86 or so, $p<0.001$), while several common metrics like the mean-median gap and partisan bias are very weakly correlated with seats won (correlation 0.18 and 0.074, respectively, both $p<0.001$).
...
In  @fig-al-maps(e), we illustrate one of the plans submitted by a plaintiff's expert in Allen.
...
In Supplementary Information D, we repeat the analysis performed here on a different counterfactual consisting of plans sampled with a redistricting simulation algorithm; the results are broadly similar: the challenged plan diﬀerentially harms Democratic voters and Black Republican voters.
...
We offer a framework for studying redistricting that brings together a focus on individual voter preferences and the causal-inference logic of counterfactuals.
...
The predicted turnout has a Spearman correlation of 0.84 ($p<0.001$) with the actual turnout, and the predicted and actual vote shares have a correlation 0.87 ($p<0.001$), across the $n=255$ precincts.
...
The data used in these analyses are based on the full 2020 Census [@census2020decennial; @census2020vtd] and all election returns for the included contests, and as such, there is no data inclusion.
All $p$-values reported in the text are two-sided.

# Data availability {.unnumbered}

All data are publicly available and are included in the replication package for the manuscript, available at <https://doi.org/10.5281/zenodo.18895840>.

# Code availability {.unnumbered}

All code is included in the replication package for the manuscript, available at <https://doi.org/10.5281/zenodo.18895840>.
...
This work contains content from both authors' dissertations [@mccartan2023computational; @kenny2025drawing].
The authors received no specific funding for this work.

# Author Contributions {.unnumbered}

C.M. and C.T.K. both contributed to the conception, design, analysis, data collection, and writing.
