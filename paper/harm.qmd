...
The harm framework combines two key ideas, which are motivated at greater length in Section @sec-motivation.
...
The combination of individual voter choices and counterfactual districting plans defines our notion of *individual harm*: informally, a voter is harmed if they are more likely to be represented by the candidate of their choice under a different districting plan.
...
A brief study of partisan gerrymandering of New Jersey congressional districts in Section @sec-nj shows how existing gerrymandering metrics can make incorrect conclusions about the magnitude and even the direction of partisan gerrymanders.
Section @sec-alabama, studies voting rights litigation for Alabama congressional districts, and uses differential harm to separate the racial and partisan effects of the enacted districting plan.
Section @sec-boston demonstrates the application of our framework to local redistricting, and quantifies the benefits of race-aware city council district boundaries to Black voters in the city of Boston.
...
Now suppose the redistricting plan in Figure @fig-minissouri(a) has been enacted.
Despite the statewide popular vote favoring Republicans, under this plan Democrats will always win two of the three seats.
In contrast, a more proportional plan @fig-minissouri(b) elects two Republicans and one Democrat.
...
```{r fig-boston-factors, fig.cap="", fig.width=7, fig.height=3.5}
#| fig.cap: |
