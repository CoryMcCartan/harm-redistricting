...
While all measures can be used with counterfactuals, our approach uses them within the definition of the measure itself.
...
For example, in Massachusetts in 2010, it was impossible to draw even one majority-Republican seat, despite 32% of voters preferring Republicans [@duch:18].
Thus the 9-0 Democratic Congressional delegation reflects the homogeneity of the state's political geography, not partisan gerrymandering.
Many existing districting metrics rely on *implicit* counterfactuals, which may be irrelevant or infeasible, as we discuss in [Appendix @sec-connections].
...
This combination of individual-level measurement and counterfactual comparison is the key innovation of our framework.
...
Here, then the partisan differential harm of plan (a) is $-41.5\%$, meaning that the average Republican voter in Mininois is $41.5$pp less likely to be represented by a Republican than the average Democrat is to be represented by a Democrat, *compared to plan (b)*.
...
To provide a neutral comparison baseline, we generate 10,000 districting plans across using the SMC algorithm of [@mccartan2020; implemented in @redist] setting constraints to favor compact plans that respect county lines.
...
Most of the other metrics, with the notable exception of the mean-median measure, agree with differential harm that each party's proposal favors itself.
...
These plans form a natural counterfactual $\tilde Q$ to use in analyzing the challenged districting plan (@fig-al-maps(d)), especially since the district court ruled in favor of the plaintiffs and found that their remedial plans satisfied the first *Gingles* criterion.
...
The first dimension primarily aligns with a traditional liberal-conservative spectrum, with progressives like Bernie Sanders and current Boston mayor Michelle Wu on the left, and centrists like Joseph R. Biden and Wu's opponent, Annissa Essaibi-George, on the right.
...
We use a simulated set of council maps as our race-neutral counterfactual.
