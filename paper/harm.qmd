...
abstract: |
  ...
  We discuss strategies for estimating harm using redistricting simulations, and demonstrate the utility of our framework through three applications to U.S. redistricting.
  Overall, harm provides a flexible way to precisely quantify redistricting's individual impacts.
keywords:
  - representation
  - redistricting
  - voting rights
...
Nearly all academic research, in contrast, has been on partisan gerrymandering, and has centered on how districting plans treat political parties, as captured in the *seats-votes curve* or notions of *partisan symmetry* [@tufte1973pb; @grofman1983measures; @king1987democratic; @king1989representation; @thomas2013estimating; @katz2020].
...
Studies of racial gerrymandering have largely operated within the legal framework for challenging such gerrymanders: the Voting Rights Act of 1965 (VRA).
...
This paper addresses these limitations by introducing a framework of *individual and differential harm*.
...
Few existing measures are focused on individual voters this way.
The exception is partisan dislocation [@defoeubarodd21], which compares the partisanship of a voter's district with the partisanship of their nearest neighbors.
...
While consideration of counterfactual plans is now commonplace with redistricting simulation algorithms [e.g., @deford2019], our approach differs from most in that it incorporates them within the definition of the harm measure itself.
Some existing districting rely on *implicit* counterfactuals, which may be irrelevant or infeasible, as we discuss in [Supplementary Information @sec-connections].
...
The *combination* of individual voter choices and counterfactual districting plans is key to our framework of *individual harm*: informally, a voter is harmed if they would be represented by their candidate of choice under a counterfactual plan.
...
A core feature of our framework is its clear separation of theoretical quantities of interest and empirical strategies for estimating them [@katz2022].
The methods section discusses the challenge of estimating harm and differential harm from observed data.
Recent advances in redistricting simulation, which can generate a large number of random districting plans that respect relevant legal constraints, are critical to this effort [@fifield2020mcmc; @deford2019; @autry2023; @mccartan2020].
...
To proceed with a legal challenge to an alleged gerrymander, a voter must demonstrate that their individual right to vote has been harmed by the enacted district boundaries [e.g., @1962baker; @issakarl98].
This requires proof that *but for* the alleged gerrymander, the voter would have been more likely to elect their chosen candidate [e.g., @2004vieth].
...
The harm measure also captures or generalizes the three *Gingles* preconditions for vote dilution litigation under the Voting Rights Act of 1965, as discussed further in [Supplementary Information @sec-connections].
...
We stress that the results here are tied to the specific counterfactual plan (b).
...
Harm involves two terms: the first measuring if a voter is not represented by their chosen candidate under $q$, and the second term measuring if they would have been represented by their chosen candidate under the counterfactual $\tilde q$.
This comparison differs from the one that underlies partisan dislocation [@defoeubarodd21], where the comparison is against a group of neighboring voters rather than an alternative plan.
We discuss this difference more in [Supplementary Information @sec-connections], and compare the two measures empirically in @sec-nj.

Notice the fundamental asymmetry in this definition: $h_i(v,q,\tilde q)\neq h_i(v,\tilde q,q)$, since $\ind\{v(i) \neq \win_i(v, q)\} \neq \ind\{v(i) \neq \win_i(v, \tilde q)\}$.
In general, $q$ harms some individuals versus $\tilde q$, and $\tilde q$ harms a distinct set of individuals versus $q$.
...
Researchers are generally interested in outcomes over multiple elections and a larger set of counterfactual plans.
...
To allow for this broader scope, we replace the fixed election $v$, counterfactual districting plan $\tilde q$, and individual voter $i$ with random variables $V\in\Elecs$, $\tilde Q\in\Plans$, and $I\in\Voters$.
The random variable $V$ represents a statistical model for possible future elections---one realization could correspond to a heavily Democratic-favoring midterm year, while another could involve incumbents under-performing particularly badly.
The random variable $\tilde Q$ represents a model for potential counterfactual districting plans.
Rarely would an analyst have a single counterfactual plan in mind: different realizations of $\tilde Q$ could correspond to different plans that respect the relevant districting criteria.
Finally, $I$ is defined to be a voter taken uniformly at random from $\Voters$, independently of $V,\tilde Q$.
...
We may also wish to study a particular election or counterfactual plan.
...
To that end, let $\Groups=\{G_1,G_2,\dots\}$ be a (random) partition of the set of voters.
In general we will not require the group identities to be fixed with respect to elections.
Formally, we have random indicator variables $\ind\{i\in G_k\}$ for all $i$ and $k$, subject to the constraint that $\sum_k \ind\{i\in G_k\}=1$ for all $i$.
These indicator variables may be correlated with $V$ and/or $\tilde Q$, in general.
This is because some identities, like partisan preference, may themselves change election-to-election.
...
The estimation of harm by group may require methods like ecological inference, as discussed in the methods section.

Zero differential harm between groups is analogous to the notion of *error rate balance* in the algorithmic fairness literature, which seeks to equalize false positive and negative rates across groups [@zafar2017fairness; @chouldechova2017fair].
There, the implicit counterfactual is a perfect predictor; the actual process harms individuals whenever it makes an incorrect prediction.
...
@fig-nj-maps shows the baseline Democratic vote share by precinct, along with the Democratic and Republican commission proposals, using data from @vest_2020.
...
The Democratic plan was enacted as the state's congressional districts.
...
For each of these sampled plans and the two proposals, we estimate the average harm and differential partisan harm versus the simulated baseline, and calculate the expected number of Democratic seats and several common partisan metrics, including partisan dislocation.
These measures, and their distribution in the baseline, are shown in @fig-nj-meas.
We calculate $\Delta_{\dem-\rep}H(q)$, so positive values correspond to a Republican-favoring plan.

The estimate of differential partisan harm aligns with the expected partisan skew of each plan, with a value of $-3.6\%$ for the Democratic and $2.0\%$ for the Republican proposal.
...
Overall, the Democratic plan has lower average harm, and a lower ranked marginal deviation [@herschlag2020quantifying].
...
Most of the other metrics agree with differential harm that each party's proposal favors itself.
The efficiency gap and partisan bias score both plans almost equally in absolute value, in comparison to declination, which identifies the Republican proposal as slightly more extreme.
However, the mean-median and partisan dislocation measures both classify the Democratic proposal as favoring Republicans, even though it produces more Democratic seats than the typical simulated plan.

The raw metrics, and these two counterintuitive results, can be better contextualized in comparison to the distribution across the sampled plans, something we discuss in [Supplementary Information @sec-app-nj].
That Supplementary Information also compares the correlations between the various metrics in the simulated sample.
...
In *Allen*, plaintiff's expert Moon Duchin submitted four such plans, one of which is shown in @fig-al-maps(e).
...
In [Supplementary Information @sec-app-alsampling], we repeat the analysis performed here on a different counterfactual consisting of plans sampled with a redistricting simulation algorithm; the results are broadly similar.
...
As we describe in the methods section, this is accomplished with an ecological inference model, using election data from @vest_2020.
...
And while modern simulation methods are a valuable starting point for defining counterfactual plans, these approaches involve analyst discretion and interpretation of often-murky legal standards, and face their own computational challenges.
Incorrect or incomplete interpretation of these standards can negatively influence the validity of the counterfactual plans.

One limitation of the harm definition is that it considers only voters; abstainers experience zero harm.
...
Thankfully, in recent years a number of algorithms have been developed which sample districting plans from specific distributions [@fifield2020mcmc; @deford2019; @autry2023; @mccartan2020].
...
Even when the choice of constraints maps onto legal requirements, it is important to ensure that the counterfactual plans are similar to the enacted plan on discretionary criteria as well.
For example, a mapdrawer may target a particular compactness metric, or draw districts that respect county boundaries, without being required to do so.
In this case, if these optional constraints are not matched in the counterfactual, it will be unclear whether any estimated harm is due to *actual* biases in a plan or the mismatch in criteria.
This includes the use of different compactness metrics in simulations than those used by the mapdrawer.
As such, it is important to not just ensure that any counterfactual samples use methods with known sampling properties, but also that the sampled plans are similar to the enacted plan on these discretionary criteria.
...
This can be thought of as a hierarchical extension of the classic uniform swing model.
...
This is done at the precinct level, since this is the most granular level that districting plans are usually built from and election returns are reported at.
...
@kenny2023widespread (Supplementary Information D) validated the model in a similar redistricting context and found it to perform well.
...
We can then identify, for each precinct, how many Democratic or Republican voters were harmed.
For this, we use the baseline count of Democratic or Republican votes.
This offers a slight simplification, since the vote share changes across random draws, and so the number of Democratic and Republican voters changes too.
...
In Boston, council districts must be equipopulous, be compact, and preserve neighborhood and community boundaries under the City of Boston charter.
...
All data are publicly available and are included in the replication package for the manuscript, available at @harmreplication.

# Code availability {.unnumbered}

All code is included in the replication package for the manuscript, available at @harmreplication.


# Acknowledgements {.unnumbered}

The authors thank Steve Ansolabehere, Matt Blackwell, Kosuke Imai, Gary King, Shiro Kuriwaki, Tyler Simko, Nick Stephanopolous, and participants of the Fall 2021 Harvard American Politics Research Workshop, Fall 2022 Harvard Applied Statistics Workshop, and Imai Research Group for helpful comments and discussion.
We thank Harrison Neely, executive director for the GOP delegation to the New Jersey Congressional Redistricting Committee, for providing a copy of the GOP's plan following an Open Public Records Act request.
The authors received no specific funding for this work.

# Author Contributions {.unnumbered}

C.Mc. and C.T.K. both contributed to the conception, design, analysis, data collection, and writing.
...
@fig-nj-pairs Supplementary Information expands on @fig-nj-meas to also show the correlations between these various measures.
...
```
@fig-alabama-harm-sample summarizes the results, which are mostly similar to the results in @sec-alabama.
